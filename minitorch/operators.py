...
def sigmoid(x: float) -> float:
    ...
    """
    # TODO: Implement for Task 0.1.
    numerator = 1.0 if x >= 0.0 else exp(x)
    denominator = 1.0 + exp(-x) if x >= 0.0 else 1.0 + exp(x)
...
def inv_back(x: float, d: float) -> float:
    ...
    # TODO: Implement for Task 0.1.
    inv_x = inv(x)
    return mul(d, neg(mul(inv_x, inv_x)))


def relu_back(x: float, d: float) -> float:
    r"If $f = relu$ compute $d \times f'(x)$"
    # TODO: Implement for Task 0.1.
    return mul(d, 1.0 if x > 0.0 else 0.0)
...
def zipWith(
    ...
    def zip_fn(ls1: Iterable[float], ls2: Iterable[float]) -> Iterable[float]:
        ...
        new_ls = [fn(ls1[i], ls2[i]) for i in range(len(ls1))]
        return new_ls
    return zip_fn
